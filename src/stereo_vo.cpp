...
StereoVO::StereoVO(cv::Mat projMatrl_, cv::Mat projMatrr_, ros::NodeHandle nh)
{
    ...
    state_sub = nh.subscribe("/shieh_decision_fsm", 10, &StereoVO::state_callback, this);
...
void StereoVO::mocap_callback(const geometry_msgs::PoseStampedConstPtr &mocap_msg)
{
    ...
    if (state_val > 1)
    {
        pose_pub.publish(mocap_pose_stamped);
    }
...
void StereoVO::state_callback(const std_msgs::Int8 &state)
{
