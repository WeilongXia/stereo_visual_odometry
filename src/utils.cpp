...
void integrateOdometryStereo(int frame_i, cv::Mat &frame_pose, const cv::Mat &rotation,
                             const cv::Mat &translation_stereo)
{
    ...
    // (translation_stereo.at<double>(2) > translation_stereo.at<double>(1)))
    /// if (scale > 0.05 && scale < 10)
    if (scale > 1e-3 && scale < 0.3) // WHY DO WE NEED THIS
    {
