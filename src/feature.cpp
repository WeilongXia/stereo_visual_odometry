#include "stereo_visual_odometry/feature.h"
...
void featureDetectionGoodFeaturesToTrack(cv::Mat image, std::vector<cv::Point2f> &points)
{
    ...
    cv::goodFeaturesToTrack(image, points, maxCorners, qualityLevel, minDistance, mask, blockSize, useHarrisDetector,
                            k);
}

// void featureTracking(cv::Mat img_1, cv::Mat img_2, std::vector<cv::Point2f>& points1, std::vector<cv::Point2f>&
// points2, std::vector<uchar>& status)
// {
//     //this function automatically gets rid of points for which tracking fails

//     std::vector<float> err;
//     cv::Size winSize=cv::Size(21,21); // TODO PARAM
//     cv::TermCriteria termcrit=cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 30, 0.01); // TODO
//     PARAM
                      ...
                      std::vector<cv::Point2f> &points_l_0_return, FeatureSet &current_features)
{
    ...
    // std::cout << "points : " << points_l_0.size() << " "<< points_r_0.size() << " "<< points_r_1.size() << " "<<
    // points_l_1.size() << " "<<std::endl;
}
                          ...
                          std::vector<cv::Point2f> &points_l_0_return, FeatureSet &current_features)
{
    ...
    cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse = cv::cuda::SparsePyrLKOpticalFlow::create(winSize, 3, 30);
    ...
    std::cerr << "calcOpticalFlowPyrLK(CUDA)  time: " << float(toc_gpu - tic_gpu) / CLOCKS_PER_SEC * 1000 << "ms"
              << std::endl;
